...
# directory
os.makedirs('./analysis/temporal_inception/inception_nochunk_new2', exist_ok=True)
...
class TemporalInception(nn.Module):
    def __init__(self, num_keypoints, num_classes=3, n_filters=64, depth=8, 
                ...
                dropout_rate=0.3, use_keypoint_masking=False):
        super().__init__()
        ...
        if use_residual:
            ...
            for i in range(depth // 2):
                ...
                if layer_idx < depth//2:
                    target_channels = n_filters * 6  
                else:
                    target_channels = n_filters * 5 
    ...
    def apply_keypoint_masking(self, x, keypoint_mask=None):
        ...
        mask_expanded = mask_expanded.expand(-1, -1, 2, frames)   # (batch, keypoints, 2, frames)
        
        x_masked = x_reshaped * mask_expanded.float()
...
class ThreeClassKeypointDataset(Dataset):
    def __init__(self, data_path, max_frames=None, is_train=True, 
            ...
            for fname in sorted(os.listdir(folder)):
                if fname.endswith(".npz"):  
                    ...
                    try:
                        ...
                        self.keypoint_stats['video_lengths'].append(frames)
                        ...
                        self.samples.append((fpath, label, class_name, vitpose_kp_count, vitpose_features))
        ...
        self.print_video_length_statistics()
        self.print_class_distribution()
        
        print(f"[{split}] Loaded {len(self.samples)} full videos")
        print(f"Class distribution: {dict(self.class_counts)}")

    def print_video_length_statistics(self):
        """Print video length statistics"""
        if self.keypoint_stats['video_lengths']:
            lengths = np.array(self.keypoint_stats['video_lengths'])
            print(f"\nVideo Length Statistics:")
            print(f"  Min frames: {np.min(lengths)}")
            print(f"  Max frames: {np.max(lengths)}")
            print(f"  Mean frames: {np.mean(lengths):.1f}")
            print(f"  Std frames: {np.std(lengths):.1f}")
    ...
    def __getitem__(self, idx):
        fpath, label, class_name, actual_vitpose_count, vitpose_features = self.samples[idx]
        ...
        if self.max_frames is not None:
            if len(data) > self.max_frames:
                data = data[:self.max_frames]
            elif len(data) < self.max_frames:
                pad_frames = self.max_frames - len(data)
                data = np.pad(data, ((0, pad_frames), (0, 0), (0, 0)), mode='constant')
        
        data = self.pad_vitpose_keypoints(data, actual_vitpose_count, vitpose_features)
        keypoint_mask = self.create_keypoint_mask(actual_vitpose_count)
        
        expected_shape = (data.shape[0], self.total_keypoints, 2)  # (frames, keypoints, 2)
        if data.shape != expected_shape:
            print(f"Shape mismatch for {fpath}: got {data.shape}, expected {expected_shape}")
            if data.shape[1] < self.total_keypoints:
                pad_kps = self.total_keypoints - data.shape[1]
                data = np.pad(data, ((0, 0), (0, pad_kps), (0, 0)), mode='constant')
  
                keypoint_mask = np.pad(keypoint_mask, (0, pad_kps), mode='constant', constant_values=False)
            elif data.shape[1] > self.total_keypoints:
                data = data[:, :self.total_keypoints, :]
                keypoint_mask = keypoint_mask[:self.total_keypoints]
                
        return (torch.FloatTensor(data), # (frames, keypoints, 2) - variable length now
                torch.LongTensor([label])[0],
                torch.LongTensor([idx])[0], 
...
class AugmentedThreeClassKeypointDataset(ThreeClassKeypointDataset):
    def __init__(self, data_path, max_frames=None, is_train=True,
        ...
        super().__init__(data_path, max_frames, is_train, vitpose_target_count, mediapipe_count)
    ...
    def __getitem__(self, idx):
        ...
        video, label, sample_id, keypoint_mask = super().__getitem__(idx)
        ...
        if self.use_augmentation and self.augmenter:
            ...
            augmented_video = self.augmenter.apply_augmentation(
                video_np, keypoint_mask_np, augment_prob
            )
            
            video = torch.FloatTensor(augmented_video)
        
        return video, label, sample_id, keypoint_mask  
        ...
        for part, indices in body_part_groups.items():
            if indices and max(indices) < len(train_importance):
                body_part_importance[part] = {
                    'train': np.mean([train_importance[i] for i in indices]),
                    'test': np.mean([test_importance[i] for i in indices])
                }
        
        results = {
            'run': run_index,
            'keypoint_importance': {
                'train': train_importance.tolist(),
                'test': test_importance.tolist(),
...
def check_existing_model(run_index, output_dir):
    ...
    model_path = f'{output_dir}/best_model_inception2_run_{run_index}.pth'
...
def main():
    ...
    print(f"Max frames per video: {max_frames if max_frames else 'Variable (full videos)'}")
    ...
    for run in range(n_runs):
        ...
        model_exists = check_existing_model(run, './analysis/temporal_inception/inception_nochunk_new2')
        
        train_dataset = AugmentedThreeClassKeypointDataset(
            data_dir, max_frames=max_frames, is_train=True,
            use_augmentation=True, augment_prob=0.3
        )
        test_dataset = ThreeClassKeypointDataset(
            data_dir, max_frames=max_frames, is_train=False
        )
        print("Dataset with full videos creation successful!")

        sample_data, sample_label, sample_id, sample_mask = train_dataset[0]
        print(f"Sample video shape: {sample_data.shape}")
        ...
        def collate_fn(batch):
            ...
            if max_frames is not None:
                ...
                videos, labels, ids, masks = zip(*batch)
                ...
                for video in videos:
                    if video.shape[0] < max_len:
                        pad_len = max_len - video.shape[0]
                        padded_video = torch.cat([video, torch.zeros(pad_len, *video.shape[1:])], dim=0)
                    else:
                        padded_video = video
        ...
        if model_exists:
            ...
            model.load_state_dict(torch.load(f'./analysis/temporal_inception/inception_nochunk_new2/best_model_inception2_run_{run}.pth'))
            ...
            for epoch in range(epochs):
                ...
                if train_loss < best_train_loss:
                    ...
                    torch.save(model.state_dict(), f'./analysis/temporal_inception/inception_nochunk_new2/best_model_inception2_run_{run}.pth')
        ...
        try:
            ...
            sample_file_path = train_dataset.samples[0][0]  
            ...
            feature_results = create_feature_importance_report(
                ...
                vitpose_feature_names, run, './analysis/temporal_inception/inception_nochunk_new2'
            )
            if feature_results:
        ...
        if not model_exists and 'train_losses' in locals() and len(train_losses) > 0:
            ...
            plt.tight_layout()
            plt.savefig(f'./analysis/temporal_inception/inception_nochunk_new2/training_curves_run_{run}.pdf', dpi=300)
            plt.close()
        else:
        ...
        plt.tight_layout()
        plt.savefig(f'./analysis/temporal_inception/inception_nochunk_new2/confusion_matrix_run_{run}.pdf', dpi=300)
        plt.close()
    

    print("\n" + "="*60)
    print("FINAL AVERAGED RESULTS (2 Runs)")
    print("="*60)
    ...
    print(f"\nAll results saved to: ./analysis/temporal_inception/inception_nochunk_new2/")

def save_comprehensive_results(results, avg_cm, std_cm, feature_results):
    """Save comprehensive results to files"""
    

    with open('./analysis/temporal_inception/inception_nochunk_new2/comprehensive_results.txt', 'w') as f:
        f.write("Enhanced Temporal Inception Model - Three Class Classification\n")
        f.write("=" * 70 + "\n\n")
    
        
        f.write("Averaged Results (2 runs):\n")
        f.write("-" * 30 + "\n")
    ...
    with open('./analysis/temporal_inception/inception_nochunk_new2/results.json', 'w') as f:
...
def create_final_visualizations(results, avg_cm, std_cm):
    ...
    plt.tight_layout()
    plt.savefig('./analysis/temporal_inception/inception_nochunk_new2/metrics_summary.pdf', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    ax.set_title('Averaged Confusion Matrix (2 runs)\nMean ± Standard Deviation', fontsize=14)
    ax.set_xlabel('Predicted Label')
    ax.set_ylabel('True Label')
    
    plt.tight_layout()
    plt.savefig('./analysis/temporal_inception/inception_nochunk_new2/final_confusion_matrix.pdf', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    ax.set_ylabel('Score')
    ax.set_title('Model Performance Summary (2 runs average)')
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    plt.savefig('./analysis/temporal_inception/inception_nochunk_new2/performance_summary.pdf', dpi=300, bbox_inches='tight')
    plt.close()
